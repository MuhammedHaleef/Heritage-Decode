...
def preprocess(img):
    ...
    img = cv2.convertScaleAbs(image, alpha=-1, beta=1)  # Set exposure to -1
    img = cv2.convertScaleAbs(img, alpha=-1, beta=50)  # Set contrast to 60
    return img
...
def upload_and_predict():
    if request.method == 'POST':
        ...
        image_file.save(temp_img)

        processed_image = preprocess(temp_img)
        ...
        if len(all_score)==0:
            os.remove(temp_img)
            return jsonify({'predicted_class':"Unable to translate" })
        else:
           ...
           os.remove(temp_img)
           return jsonify({'predicted_class':translation })
