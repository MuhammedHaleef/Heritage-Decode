...
dataset_path = 'C:/archi/train5'

# Function to load images and labels from the dataset
def load_dataset(dataset_path):
    images = []
    labels = []
    label_names = sorted(os.listdir(dataset_path))  # Assuming each subfolder represents a class
    ...
    for label, label_name in enumerate(label_names):
        ...
        for image_file in os.listdir(label_path):
            ...
            # thresh = preprocess.applyThreshold(img2)[1]
            # cleaned = preprocess.clean(1,thresh)
            img1 = np.array(img2)

            # image = np.array(Image.open(image_path).convert('GRAY'))
            images.append(img1)
            labels.append(label)
    print("k")
...
def save_imgs(epoch):
    ...
    gen_imgs = generator.predict(noise+0.1)
    ...
    for i in range(r):
        for j in range(c):
            ...
            axs[i,j].axis('off')
            cnt += 1
    fig.savefig("C:/archi\w11/mnist_%d.png" % epoch)
    plt.close()
...
generator.save('generator_model.keras')  #Test the model on GAN4_predict...
